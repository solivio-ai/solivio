...
import { normalizeCustomerName, upsertCustomerByName } from "../customers/customerRepository";
import { db } from "../database/db";
import { findIdsBySku } from "../products/productRepository";
import type { InsertOfferItemData } from "./offerRepository";
import { insertOffer, insertOfferItems } from "./offerRepository";
...
export async function importOffers(records: unknown[]): Promise<{
  ...
  await db.transaction(async (tx) => {
    const skuToProductId = await findIdsBySku(allSkus, tx);

    for (const order of valid) {
      const customer = await upsertCustomerByName(order.customerName, "import", tx);
