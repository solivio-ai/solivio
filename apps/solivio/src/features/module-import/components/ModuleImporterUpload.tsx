...
import type { ImportTarget } from "@solivio/sdk";
...
type Props = {
  ...
  endpoint: string;
  inputId: string;
  rowErrorField?: RowErrorField;
  target: ImportTarget;
};

export function ModuleImporterUpload({
  accept,
  cardDescription,
  cardTitle,
  endpoint,
  ...
  const t = useTranslations("ModuleImport");
              ...
              {status.kind === "done" ? (
                <>
                  <StatusNotice tone="success" icon={<CheckCircle2 size={16} aria-hidden="true" />}>
                    {t(`card.importDone.${target}`, { count: status.count })}
                  </StatusNotice>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {status.errors.map((e, i) => (
                    <StatusNotice
                      ...
                      icon={<AlertTriangle size={16} aria-hidden="true" />}
                    >
                      {rowErrorField && e[rowErrorField] ? `[${e[rowErrorField]}] ` : ""}
                      {e.message}
                    </StatusNotice>
                  ))}
                </div>
