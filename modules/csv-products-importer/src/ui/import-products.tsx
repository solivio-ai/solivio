...
const copy = {
  ...
  saved: { en: "Saved {count} product(s).", pl: "Zapisano {count} produktów." },
};
...
export function mount(element: HTMLElement, context: ModuleUiMountContext): () => void {
  ...
  root.render(<ProductImportUpload context={context} />);
  return () => root.unmount();
}

function ProductImportUpload({ context }: { context: ModuleUiMountContext }) {
  ...
  const inputId = "csv-products-module-upload";
    ...
    <section className="grid gap-3">
      <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
        <div className="flex flex-row items-center gap-2 px-4 pt-4 pb-1">
          ...
          <h2 className="text-base font-semibold">{t(title, locale, "Catalog upload")}</h2>
        </div>
        <div className="grid gap-3 p-4">
          <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground">
            {t(description, locale, "Pick a file and import it into the product catalog.")}
          </p>
